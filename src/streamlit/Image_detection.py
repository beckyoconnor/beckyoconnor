...
SQL = '''select GET_PRESIGNED_URL(@data_stg, RELATIVE_PATH) URL FROM DIRECTORY(@data_stg) URL SAMPLE (1 rows);'''
...
if choosefile:
    ...
    def parse_image(parse):
        df2 = sp_session.sql(SQL2)
        df2 = df2.select(\
